const tenisrun = [
    ...
    },
    {
        ...
        Lançamento: '2019',
        Categoria: 'Dia a dia',
        Indicacao: 'Tênis curinga, indicado para treinos diários, treinos de velocidade e provas',
        Pisada: 'Neutra',
        Drop: '4mm',
    ...
    },
    {
        ...
        Lançamento: '2021',
        Categoria: 'Dia a dia',
        Indicacao: 'Modelo indicado para treinos diários, treinos de velocidade e provas',
        Pisada: 'Neutra/Supinada',
        Drop: '10mm',
    ...
    },
    {
        ...
        Indicacao: 'Tênis indicado pra caminhadas, corridas leves e treinos de academia',
        Pisada: 'Neutra',
        Drop: '8mm',
...
const findAlltenisRunService = () => {
    return tenisrun;
};

const findByIdtenisRunService = (idPar) => {
    return tenisrun.find((tenis) => tenis.id === idPar);
};

const createtenisRunService = (newTenis) => {
    const newId = tenisrun.length +1;
    newTenis.id = newId;
    tenisrun.push(newTenis);
    return newTenis;
};

const updatetenisRunService = (id, tenisEd) => {
    tenisEd['id'] = id;
    const tenisIndex = tenisrun.findIndex((tenisUp) => tenisUp.id == id);
    tenisrun[tenisIndex] = tenisEd;
    return tenisEd;
  };

const deletetenisRunService = (id) => {
    const tenisIndex = tenisrun.findIndex((tenisId) => tenisId.id == id);
    return tenisrun.splice(tenisIndex, 1);
};
